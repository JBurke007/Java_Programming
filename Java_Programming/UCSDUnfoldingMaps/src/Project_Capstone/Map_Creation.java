...
public class Map_Creation extends PApplet{
	...
	private void shadeCountries() {
		for (Marker marker : countryMarkers) {
			...
			// condition to make sure that we only set the color of a country poly if we have data
			if (lifeExpMap.containsKey(countryId)) {
				float lifeExp = lifeExpMap.get(countryId);
				// Encode value as brightness (values range: 40-90)
				int colorLevel = (int) map(lifeExp, 40, 90, 10, 255);
				// Because we are using red to indicate the level of poor LE, the countries with
				// lower life expectancy will reduce the red 255 value by the smallest amount,
				// leaving the country polygon closer to bright red.
